  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const [projRes, campRes] = await Promise.all([
        fetch("/api/partner/project", { headers: { Authorization: `Bearer ${session.access_token}` } }),
      ...
      if (projData.project) {
        ...
        // Fetch campaigns
        const slug = projData.project.slug;
        const c = await fetch(`/api/campaigns/by-project/${slug}`).then((r) => r.json());
        setCampaigns(c.campaigns ?? []);
        const edits: typeof campaignEdits = {};
        for (const cam of c.campaigns ?? []) {
          edits[cam.id] = { name: cam.name, goal: String(cam.goal), description: cam.description ?? "", status: cam.status };
        }
            ...
            <div className="bg-white rounded-2xl p-6 shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
              ...
              <form onSubmit={saveProject} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="url" value={editImage} onChange={(e) => setEditImage(e.target.value)}
                    placeholder="https://…"
                        ...
                        {isEditing ? (
                          ...
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button onClick={() => setEditingCampaign(c.id)} className="text-xs text-[#24B5CB] font-medium hover:underline">Edit</button>
                            </div>
                          </div>
                        )}
                      </div>
              ...
              <form onSubmit={createCampaign} className="space-y-3">
                ...
                <div className="flex gap-2">
                  <input type="number" value={newCampaignGoal} onChange={(e) => setNewCampaignGoal(e.target.value)} required placeholder="Goal ($)" min="1"
                    className="flex-1 border border-[#D1D5DB] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-[#24B5CB]" />
                </div>
